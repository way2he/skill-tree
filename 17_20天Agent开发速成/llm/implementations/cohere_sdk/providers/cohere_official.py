...
基于 cohere SDK 实现，使用 API Key 鉴权。
...
class CohereOfficialClient(CohereClient):
    """
    Cohere 官方 SDK 客户端

    使用 cohere.Client 进行模型调用。

    Args:
        api_key: API Key
        model: 模型名称，如 "command-r", "command-r-plus"
        system_prompt: 系统提示词
    ...
    def _init_client(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "cohere 未安装，请运行: pip install cohere"
            )
    ...
    def generate(self, prompt: str, **kwargs: Any) -> str:
        ...
        Example:
            >>> client = CohereOfficialClient(api_key="your-api-key")
        ...
        if self.system_prompt:
            params["preamble"] = self.system_prompt
