...
class GLMProvider(BaseProviderClient):
    """
    智谱AI GLM 客户端
    ...
    使用示例:
        # 同步调用
        client = GLMProvider(api_key="your-api-key", model="glm-4")
        response = client.generate("你好，请介绍一下自己")

        # 异步调用
        import asyncio
        async def main():
            client = GLMProvider(api_key="your-api-key")
            response = await client.agenerate("你好")
            async for chunk in client.agenerate_stream("讲个故事"):
                print(chunk, end="", flush=True)

    Args:
        api_key: 智谱AI API 密钥，可通过环境变量 ZHIPUAI_API_KEY 设置
        model: 模型名称，如 "glm-4", "glm-4v", "glm-3-turbo"
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("API Key 未设置，请设置 ZHIPUAI_API_KEY 环境变量")
