# -*- coding: utf-8 -*-
"""
同步/异步 Meta Llama 客户端（通过第三方 API 如 Together AI）
"""
...
class MetaProvider(BaseProviderClient):
    """
    同步/异步 Meta Llama 客户端（通过 Together AI）
    API 文档: https://docs.together.ai/reference

    使用示例:
        client = MetaProvider(api_key="your-together-api-key")
    ...
    Args:
        api_key: Together AI API 密钥，可通过环境变量 TOGETHER_API_KEY 设置
        model: 模型名称，如 "meta-llama/Llama-3-70b-chat-hf", "meta-llama/Llama-3-8b-chat-hf"
    ...
    Note:
        Meta 不直接提供 API 服务，需要通过第三方平台（如 Together AI、Fireworks AI、Groq 等）调用
        这里以 Together AI 为例
    """
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("API Key 未设置，请设置 TOGETHER_API_KEY 环境变量")
