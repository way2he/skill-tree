# -*- coding: utf-8 -*-
"""
腾讯混元(Hunyuan) Provider

支持同步和异步 API 调用，使用腾讯云签名认证。
"""
...
class HunyuanProvider(BaseProviderClient):
    """
    腾讯混元 Hunyuan Provider
    ...
    使用示例:
        ...
        response = provider.generate("你好，请介绍一下自己")

        # 异步调用
        import asyncio
        async def main():
            response = await provider.agenerate("你好，请介绍一下自己")
    ...
    # 类属性：厂商名称
    PROVIDER_NAME: str = "hunyuan"
    # 类属性：默认模型
    DEFAULT_MODEL: str = "hunyuan-turbo"
    ...
    def __init__(
        ...
        if not self.secret_id or not self.secret_key:
            raise ValueError(
                "Secret Id 和 Secret Key 未设置，请设置 TENCENT_SECRET_ID 和 TENCENT_SECRET_KEY 环境变量"
            )
    ...
    def _get_base_payload(self, prompt: str, **kwargs: Any) -> dict[str, Any]:
        """
        获取基础请求 payload
    ...
    def generate(self, prompt: str, **kwargs: Any) -> str:
        ...
        payload = self._get_base_payload(prompt, **kwargs)
