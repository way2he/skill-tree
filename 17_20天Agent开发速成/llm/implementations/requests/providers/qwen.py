...
class QwenProvider(BaseProviderClient):
    """
    阿里云通义千问客户端
    ...
    使用示例:
        # 同步调用
        client = QwenProvider(api_key="your-api-key", model="qwen-plus")
        response = client.generate("你好，请介绍一下自己")

        # 异步调用
        import asyncio
        async def main():
            client = QwenProvider(api_key="your-api-key")
            response = await client.agenerate("你好")
            async for chunk in client.agenerate_stream("讲个故事"):
                print(chunk, end="", flush=True)

    Args:
        api_key: 阿里云 API 密钥，可通过环境变量 DASHSCOPE_API_KEY 设置
