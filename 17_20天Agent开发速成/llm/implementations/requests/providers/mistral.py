...
class MistralProvider(BaseProviderClient):
    """
    同步 Mistral AI 客户端
    API 文档: https://docs.mistral.ai/

    使用示例:
        client = MistralProvider(api_key="your-api-key", model="mistral-large-latest")
        response = client.generate("Hello, how are you?")

    Args:
        api_key: Mistral API 密钥，可通过环境变量 MISTRAL_API_KEY 设置
        model: 模型名称，如 "mistral-large-latest", "mistral-small-latest", "mistral-medium-latest"
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("API Key 未设置，请设置 MISTRAL_API_KEY 环境变量")
        self.model = model
