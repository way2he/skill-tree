...
API 文档: https://platform.stepfun.com/
"""
...
class StepfunProvider(BaseProviderClient):
    """
    阶跃星辰 Provider 客户端

    使用示例（同步）:
        provider = StepfunProvider(api_key="your-api-key")
    ...
    Args:
        api_key: 阶跃星辰 API 密钥，可通过环境变量 STEPFUN_API_KEY 设置
        model: 模型名称，如 "step-2", "step-1.5"
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("API Key 未设置，请设置 STEPFUN_API_KEY 环境变量")
