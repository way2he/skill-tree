...
class TogetherProvider(BaseProviderClient):
    """
    同步 Together AI 客户端（支持开源模型）
    API 文档: https://docs.together.ai/reference

    使用示例:
        client = TogetherProvider(api_key="your-api-key", model="mistralai/Mixtral-8x7B-Instruct-v0.1")
        response = client.generate("Hello, how are you?")

    Args:
        api_key: Together AI API 密钥，可通过环境变量 TOGETHER_API_KEY 设置
        model: 模型名称，如 "mistralai/Mixtral-8x7B-Instruct-v0.1", "meta-llama/Llama-3-70b-chat-hf"
    ...
    async def agenerate_stream(self, prompt: str, **kwargs: Any) -> AsyncIterator[str]:
        ...
        async with httpx.AsyncClient() as client:
            async with client.stream(
                ...
                resp.raise_for_status()
                async for raw in resp.aiter_lines(decode_unicode=True):
                    if not raw or not raw.startswith("data:"):
                        continue
                    data = raw[5:].strip()
                    if data == "[DONE]":
...
# 向后兼容：保留旧的类名作为别名
TogetherClient = TogetherProvider
