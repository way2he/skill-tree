...
class CohereProvider(BaseProviderClient):
    """
    Cohere LLM 客户端
    API 文档: https://docs.cohere.com/

    使用示例:
        client = CohereProvider(api_key="your-api-key", model="command-r-plus")
    ...
    def __init__(
        ...
        Args:
            api_key: API 密钥，默认从环境变量 COHERE_API_KEY 读取
            model: 模型名称，默认 "command-r-plus"
    ...
    async def agenerate_stream(self, prompt: str, **kwargs: Any) -> AsyncIterator[str]:
        """
        异步流式生成（Cohere v1/chat NDJSON 流）
