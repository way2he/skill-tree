...
class WenxinProvider(BaseProviderClient):
    """
    百度文心一言 Provider
    ...
    # 类属性：厂商名称
    PROVIDER_NAME: str = "wenxin"
    # 类属性：默认模型
    DEFAULT_MODEL: str = "ernie-4.0-8k-latest"
    ...
    def __init__(
        ...
        if not self.api_key or not self.secret_key:
            raise ValueError(
                "API Key 和 Secret Key 未设置，请设置 WENXIN_API_KEY 和 WENXIN_SECRET_KEY 环境变量"
            )
    ...
    async def _aget_access_token(self) -> str:
        """
        获取 access_token（异步）
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(auth_url, params=params, timeout=self.timeout)
            response.raise_for_status()
            self._access_token = response.json()["access_token"]
            return self._access_token
