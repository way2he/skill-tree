...
class YiProvider(BaseProviderClient):
    """
    零一万物 Provider

    使用示例（同步）:
        client = YiProvider(api_key="your-api-key", model="yi-large")
    ...
    Args:
        api_key: 零一万物 API 密钥，可通过环境变量 YI_API_KEY 设置
        model: 模型名称，如 "yi-large", "yi-medium", "yi-small", "yi-vision", "yi-coder"
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("API Key 未设置，请设置 YI_API_KEY 环境变量")
