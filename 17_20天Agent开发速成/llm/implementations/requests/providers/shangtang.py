...
API 文档: https://platform.sensenova.cn/
"""
...
class ShangtangProvider(BaseProviderClient):
    """
    商汤科技 Provider 客户端

    使用示例（同步）:
        provider = ShangtangProvider(api_key="your-api-key")
    ...
    Args:
        api_key: 商汤科技 API 密钥，可通过环境变量 SHANGTANG_API_KEY 设置
        model: 模型名称，如 "SenseChat-5.5", "SenseChat-5.0"
    ...
    def __init__(
        ...
        self.api_key = api_key or os.getenv("SHANGTANG_API_KEY")
        if not self.api_key:
            raise ValueError("API Key 未设置，请设置 SHANGTANG_API_KEY 环境变量")
...
ShangtangClient = ShangtangProvider
