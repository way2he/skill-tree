...
class GLMClient(OpenAICompatibleClient):
    ...
    使用智谱AI开放平台 API，支持 glm-4、glm-4-flash 等模型。
    通过设置 ZHIPUAI_API_KEY 环境变量或传入 api_key 参数进行认证。
    ...
    ENV_API_KEY: str = "ZHIPUAI_API_KEY"
    ...
    def __init__(
        ...
        Args:
            api_key: API 密钥，若为 None 则从环境变量 ZHIPUAI_API_KEY 获取
            model: 模型名称，默认为 glm-4
            base_url: API 基础地址，默认为 https://open.bigmodel.cn/api/paas/v4
            system_prompt: 系统提示词
