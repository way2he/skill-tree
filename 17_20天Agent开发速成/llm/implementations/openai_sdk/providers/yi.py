...
class YiClient(OpenAICompatibleClient):
    """
    零一万物客户端

    基于 OpenAI SDK 兼容协议，调用零一万物提供的 Yi 系列大语言模型。
    ...
    PROVIDER_NAME: str = "yi"
    DEFAULT_BASE_URL: str = "https://api.lingyiwanwu.com/v1"
    DEFAULT_MODEL: str = "yi-large"
    ENV_API_KEY: str = "YI_API_KEY"
    ...
    def __init__(
        ...
        Args:
            api_key: API 密钥，若为 None 则从环境变量 YI_API_KEY 读取
            model: 模型名称，默认使用 yi-large
            base_url: API 基础地址，默认使用 https://api.lingyiwanwu.com/v1
            system_prompt: 系统提示词
