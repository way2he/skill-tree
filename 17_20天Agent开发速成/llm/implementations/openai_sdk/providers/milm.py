...
class MiLMClient(OpenAICompatibleClient):
    """
    小米 MiLM 客户端

    使用小米大模型 API，支持 MiLM2 等模型。
    通过设置 XIAOAI_API_KEY 环境变量或传入 api_key 参数进行认证。
    ...
    PROVIDER_NAME: str = "milm"
    DEFAULT_BASE_URL: str = "https://api.xiaoai.com/v1"
    DEFAULT_MODEL: str = "MiLM2"
    ENV_API_KEY: str = "XIAOAI_API_KEY"
    ...
    def __init__(
        ...
        Args:
            api_key: API 密钥，若为 None 则从环境变量 XIAOAI_API_KEY 获取
            model: 模型名称，默认为 MiLM2
            base_url: API 基础地址，默认为 https://api.xiaoai.com/v1
            system_prompt: 系统提示词
