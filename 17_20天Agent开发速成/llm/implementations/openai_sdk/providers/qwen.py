...
class QwenClient(OpenAICompatibleClient):
    ...
    使用阿里云 DashScope 兼容模式 API，支持 qwen-plus、qwen-turbo 等模型。
    ...
    def __init__(
        ...
        Args:
            api_key: API 密钥，若为 None 则从环境变量 DASHSCOPE_API_KEY 获取
            model: 模型名称，默认为 qwen-plus
            base_url: API 基础地址，默认为 https://dashscope.aliyuncs.com/compatible-mode/v1
            system_prompt: 系统提示词
