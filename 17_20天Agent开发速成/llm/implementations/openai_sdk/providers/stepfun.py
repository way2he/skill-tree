...
"""
阶跃星辰 OpenAI SDK 客户端
通过 OpenAI 兼容协议调用阶跃星辰的 Step 系列模型
"""
...
class StepfunClient(OpenAICompatibleClient):
    """
    阶跃星辰客户端

    基于 OpenAI SDK 兼容协议，调用阶跃星辰提供的 Step 系列大语言模型。
    ...
    def __init__(
        ...
        Args:
            api_key: API 密钥，若为 None 则从环境变量 STEPFUN_API_KEY 读取
            model: 模型名称，默认使用 step-2
            base_url: API 基础地址，默认使用 https://api.stepfun.com/v1
            system_prompt: 系统提示词
