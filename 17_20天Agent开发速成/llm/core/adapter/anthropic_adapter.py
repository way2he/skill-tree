"""
LLM 统一接口层 - Anthropic 适配器
适配 anthropic/ 目录，处理 dict 返回值转换
"""
...
class AnthropicLLMAdapter(BaseLLMAdapter):
    ...
    适配 anthropic/ 目录的客户端
    主要处理 generate_json 返回 dict 的情况
    """
    
    def __init__(self, inner_client: Any, provider_name: str = "anthropic"):
        """初始化 Anthropic 适配器
        
        Args:
            inner_client: 被适配的 Anthropic 客户端
            provider_name: 提供者名称
        """
    ...
    def generate_json(self, prompt: str, schema: Any = None, **kwargs) -> str:
        ...
        Anthropic 的 generate_json 可能返回 dict，需要转换为 str
